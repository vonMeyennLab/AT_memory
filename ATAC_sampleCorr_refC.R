...
rawCounts <- data.frame(readRDS("ATAC_AdipoERCre_refC_filtPeakCounts.rds"))
...
rawCounts[, "C_short_ATAC_AdipoERCre"] <- rowMeans(rawCounts[,c(1,5,9)])

# Add sample information
group <- data.frame(condition, experiment, row.names = samples)
group["C_short_ATAC_AdipoERCre",] <- c("C", "short")
...
xx <- cor(normCounts, use="complete.obs")
...
ggplot(xx) + 
  geom_bar(aes(cond, C_short_ATAC_AdipoERCre, fill = cond), 
           position = "dodge", stat = "summary", fun="mean") +
  #scale_fill_manual(values = my_col) + facet_wrap(. ~ variable, strip.position="bottom", ncol=4) +
  geom_point(aes(x=cond,y=C_short_ATAC_AdipoERCre,group=cond),
...
my_sum <- xx %>%
  ...
  summarise( 
    n=n(),
    mean=mean(C_short_ATAC_AdipoERCre),
    sd=sd(C_short_ATAC_AdipoERCre)
  ) %>%
  mutate( se=sd/sqrt(n))  %>%
...
write.csv(xx, "ATAC_refC_corrCoeff.csv", row.names = FALSE)
