...
for(i in (1:nrow(metadata)))
{
  ...
  peakInfo$exp          <- sampleInfo[2]
  peakInfo$histone      <- sampleInfo[3]
  peakInfo$replicate    <- sampleInfo[4]
  peakInfo$timePoint    <- sampleInfo[5]
  ...
  peakN       <- data.frame(sampleID = metadata[i,"ID"],
                            condition = sampleInfo[1], exp = sampleInfo[2], histone = sampleInfo[3], replicate = sampleInfo[4], timePoint = sampleInfo[5],
...
saveRDS(list(peakSummary = peakSummary, peakN = peakN), "CnT_filtPeaks.rds")
...
saveRDS(peakAnno, "CnT_filtPeakAnno.rds")
